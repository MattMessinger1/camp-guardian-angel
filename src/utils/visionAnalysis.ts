...
export async function analyzePageWithVision(
  ...
  try {
    ...
    console.log('🔍 Request payload:', { sessionId, model, screenshotLength: cleanScreenshot.length, isolationTest });
...
export async function testVisionAnalysis(model: string = 'gpt-4o-mini', isolationTest: boolean = false) {
  ...
  try {
    ...
    const analysis = await analyzePageWithVision(testScreenshot, 'test-session', model, isolationTest);
