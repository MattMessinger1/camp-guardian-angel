...
serve(async (req) => {
  ...
  try {
    ...
    const { token, action } = await req.json(); // action: 'approve' | 'decline'

    console.log(`[APPROVAL-SUBMIT] Processing ${action} for token: ${token.substring(0, 10)}...`);

    // Verify and decode token
    const tokenData = await verifySecureToken(token);
    
    if (!tokenData || tokenData.type !== 'approve') {
    ...
    if (action === 'approve') {
      ...
      if (tokenData.registration_id) {
        ...
        console.log(`[APPROVAL-SUBMIT] Approved and resumed registration: ${tokenData.registration_id}`);
      }

      return new Response(
        JSON.stringify({
          success: true,
          message: 'Registration approved. Process will continue.',
          action: 'approved',
      ...
      if (tokenData.registration_id) {
        await supabase
          ...
          .update({ 
            ...
            result_message: 'Declined by parent via SMS link'
          })
          .eq('id', tokenData.registration_id);

        console.log(`[APPROVAL-SUBMIT] Declined registration: ${tokenData.registration_id}`);
      }

      return new Response(
        JSON.stringify({
          success: true,
          message: 'Registration declined and cancelled.',
          action: 'declined',
          registration_id: tokenData.registration_id
        }),
        { headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
