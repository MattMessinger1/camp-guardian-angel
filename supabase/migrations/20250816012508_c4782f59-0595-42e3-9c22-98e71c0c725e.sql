-- Enhanced hybrid search function with all required parameters and performance optimizations
...
WITH base AS (
  ...
  WHERE 
    (p_city IS NULL OR mv.city ILIKE p_city || '%') AND
    (p_state IS NULL OR mv.state ILIKE p_state || '%')
),
...
freshness_scores AS (
  SELECT
    ...
    CASE
      ...
      ELSE GREATEST(0.0, 0.2 * (1.0 - EXTRACT(EPOCH FROM (now() - MAX(s.last_verified_at))) / (30 * 24 * 3600))) -- 30 day decay
    END as fresh_score
  FROM sessions s
...
availability_scores AS (
  SELECT
    ...
    CASE
      ...
      WHEN p_availability = 'open' AND s.availability_status = 'open' THEN 0.1
      WHEN p_availability = 'any' THEN 0.05
      ELSE 0.0
    END as avail_score
  FROM sessions s
  WHERE s.activity_id IN (SELECT b.activity_id FROM base b)
...
scored AS (
  SELECT
    ...
    (COALESCE(ks.kw_score, 0.0) + COALESCE(vs.vec_score, 0.0) + COALESCE(fs.fresh_score, 0.0) + COALESCE(avs.avail_score, 0.0)) as total_score
  FROM base b
...
GRANT EXECUTE ON FUNCTION public.search_hybrid(text, vector(1536), text, text, int, int, date, date, numeric, text, int, int) TO anon, authenticated, service_role;

-- Create a query cache table for performance
...
-- RLS for cache (edge functions only)
ALTER TABLE public.search_cache ENABLE ROW LEVEL SECURITY;
CREATE POLICY "Edge functions can manage search cache" ON public.search_cache FOR ALL USING (true);
