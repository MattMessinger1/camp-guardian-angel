...
DROP MATERIALIZED VIEW IF EXISTS public.registration_funnels_mv CASCADE;

-- Create materialized view for registration funnel analytics
CREATE MATERIALIZED VIEW public.registration_funnels_mv AS
WITH funnel_data AS (
  SELECT 
    COALESCE(s.provider_id::text, 'no-provider') as provider_domain,
  ...
  GROUP BY COALESCE(s.provider_id::text, 'no-provider'), DATE_TRUNC('day', r.requested_at)
),
