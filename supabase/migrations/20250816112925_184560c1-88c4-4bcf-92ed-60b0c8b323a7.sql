...
-- Create RLS policy (though materialized views don't enforce RLS by default)
ALTER TABLE public.mv_sessions_search ENABLE ROW LEVEL SECURITY;

-- Allow all authenticated users to read the materialized view
CREATE POLICY "mv_sessions_search_read" 
ON public.mv_sessions_search 
FOR SELECT 
USING (true);

-- Edge functions can also read for admin operations
CREATE POLICY "Edge functions can read sessions search mv"
ON public.mv_sessions_search
FOR SELECT
USING (true);