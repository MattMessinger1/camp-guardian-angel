...
-- Migration: 20250908_skiclubpro_and_locator

-- Add platform enum value (if enum is used)
DO $$ BEGIN
...
-- Seed provider_profiles row if missing
INSERT INTO provider_profiles (id, name, platform, domain_patterns, login_type, captcha_expected, notes)
SELECT gen_random_uuid(), 'SkiClubPro', 'skiclubpro', ARRAY['*.skiclubpro.team'], 'email_password', false, 'Ski/club registration platform'
WHERE NOT EXISTS (SELECT 1 FROM provider_profiles WHERE platform='skiclubpro');