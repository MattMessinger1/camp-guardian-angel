-- Improve search_hybrid function to better prioritize location relevance
...
-- Location matching boost - NEW: Strong boost for exact location matches
location_scores AS (
  SELECT
    b.activity_id,
    CASE
      WHEN p_city IS NOT NULL AND p_state IS NOT NULL AND 
           lower(b.city) = lower(p_city) AND lower(b.state) = lower(p_state) THEN 0.6 -- Exact city+state match
      WHEN p_city IS NOT NULL AND lower(b.city) = lower(p_city) THEN 0.4 -- Exact city match
      WHEN p_state IS NOT NULL AND lower(b.state) = lower(p_state) THEN 0.2 -- State match
      ELSE 0.0
    END as loc_score
  FROM base b
),
...
scored AS (
  SELECT
    ...
    -- Total score now includes location boost as primary factor
    (COALESCE(ls.loc_score, 0.0) + COALESCE(ks.kw_score, 0.0) + COALESCE(vs.vec_score, 0.0) + COALESCE(fs.fresh_score, 0.0) + COALESCE(avs.avail_score, 0.0)) as total_score
  FROM base b
...
-- Primary sort by total score (includes location boost), secondary by session start date
