...
ADD COLUMN confidence_score NUMERIC DEFAULT 0.8 CHECK (confidence_score >= 0.0 AND confidence_score <= 1.0);

-- Create TOS monitoring schedule table
CREATE TABLE public.tos_monitoring_schedule (
...
CREATE TABLE public.tos_change_log (
  id UUID NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
  url TEXT NOT NULL,
  hostname TEXT NOT NULL,
...
CREATE INDEX idx_tos_monitoring_next_check ON public.tos_monitoring_schedule(next_check);
CREATE INDEX idx_tos_monitoring_hostname ON public.tos_monitoring_schedule(hostname);
CREATE INDEX idx_tos_monitoring_status ON public.tos_monitoring_schedule(status);

CREATE INDEX idx_tos_change_log_hostname ON public.tos_change_log(hostname);
CREATE INDEX idx_tos_change_log_detected_at ON public.tos_change_log(change_detected_at);
CREATE INDEX idx_tos_change_log_significance ON public.tos_change_log(significance);

CREATE INDEX idx_partnership_interactions_partnership_id ON public.partnership_interactions(partnership_id);
CREATE INDEX idx_partnership_interactions_date ON public.partnership_interactions(interaction_date);
CREATE INDEX idx_partnership_interactions_type ON public.partnership_interactions(interaction_type);
...
-- Insert some initial monitoring schedules for key camp providers
INSERT INTO public.tos_monitoring_schedule (hostname, url, frequency) VALUES
('active.com', 'https://active.com/terms', 'monthly'),
('campwise.com', 'https://campwise.com/terms', 'weekly'),
('ymca.org', 'https://ymca.org/terms', 'monthly'),
('campminder.com', 'https://campminder.com/terms', 'weekly'),
('daysmart.com', 'https://daysmart.com/terms', 'monthly'),
('jackrabbitclass.com', 'https://jackrabbitclass.com/terms', 'monthly');
