...
test.describe('MVP workflow end-to-end', () => {
  ...
  test.skip('2. Session search shows results', async ({ page }) => {
    await page.goto('/');
    await expect(page.getByText(/Session|Camp|Age/i)).toBeVisible();
  });

  test.skip('3. Reservation form loads', async ({ page }) => {
    await page.goto('/reservation-holds');
    await expect(page.getByText(/Reservation/i)).toBeVisible();
  });

  test.skip('4. Stripe payment flow mocked', async ({ page }) => {
    await page.goto('/');
    await expect(page.getByText(/Payment|Card/i)).toBeVisible();
  });

  test.skip('5. Manual backup page shows link or reason', async ({ page }) => {
